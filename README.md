...
### Descrição da API
Desenvolver uma API que atenda à modelagem disponibilizada abaixo (não é necessário se limitar a ela), está API deve ser construída com a finalidade de gerenciar atividades de um projeto que podem ser visualizadas por membros desse projeto adicionados pelo administrador.
...
## item 3
Como apresentado no diagrama as atividades ficam agrupadas em tópicos, onde possuem um campo chamado index_on ordenado em ordem crescente (0,1,2,3,4...) que guarda a sua posição da atividade dentro do tópico, deve ser possível mudar sua posição das atividades no tópico, levando em consideração que duas atividades não podem ESTAR na mesma posição em um tópico e nem existirem entre a ordenação como (0,2,3,4,15) ou números negativos.
## item 4
...
## item 8
- Ao excluir um projeto não se pode ter acesso aos demais dados do projeto como suas atividades, membros e tópicos.
...
- Ao se excluir todos os membros do projeto, todos os dados do projeto devem ser excluídos
## item 9
 - Dentre as rotas da aplicação, construir uma rota que retorne as atividades em **andamento**(atividades sem data definida ou dentro do prazo), **atrasadas** e **concluídas** do membro do projeto. Lembrando que o usuário deve ser membro da atividade para que esta entre no calculo.
...
- Cobertura de código nos >= 70%;
- README descritivo e com o passo a passo (comandos) para executar a API.

### Itens que nos deixam com mais vontade de ter você no nosso time:
- Seu repositório contém algum tipo de script que cria o BANCO de dados e toda a "infra" necessária para a execução.
...
- Seu repositório tem algum tipo de automação que compile o projeto e rode os testes.
- A sua API possui documentação (eu li swagger?).
- Anexar um arquivo json das rotas criadas no insomnia.
### Envio
